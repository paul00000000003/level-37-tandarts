...
class AppointmentInDay extends React.Component {
  ...
  render() {
    ...
    this.props.dentists.forEach((element) => {
      if (
        element.isSick === "J" &&
        element.firstName.toUpperCase() === this.props.dentist.toUpperCase()
      )
        tandartsZiek = true;
    });
    let text = "";
    if (tandartsZiek) text = "appointmentRood";
      ...
      <li className={text}>
        ...
        <div className="containerknop">
          <button
            className="knop"
            onClick={(e) =>
              this.props.afspraakVerwijderen(
                this.props.patient,
                this.props.dentist,
                this.props.time
              )
