...
const rangschikOpTijdStip=(rel) => rel.sort(function(a, b) {
...
const zoekOudRecord=(appointments,day,dentist,time ) => {appointments.forEach((element,index)=> {if ((element.day==day) && 
...
const verwijderRecord=(appointments,indexOud) =>  appointments.splice(indexOud,1)


const voegRecordToe= (appointments,idOud,dentist,patient,newDay,newTime) => appointments.push({id       : idOud,
                                                                                               dentist  : dentist,
                                                                                               patient  : patient, 
                                                                                               day      : newDay,
                                                                                               time     : newTime})
...
class Day extends React.Component {
  ...
  handleClickWijzigen(e)
  {
    ...
    { indexOud=0 
      ...
      { 
        let idOud=0
        let oudRecord=zoekOudRecord(this.props.appointments,this.state.day,this.state.dentist,this.state.time)
        indexOud=oudRecord[0]
        idOud=oudRecord[1]
      
        if (indexOud !==0)
          {verwijderRecord(this.props.appointments,indexOud)
           voegRecordToe(this.props.appointments,idOud,this.state.dentist,this.state.patient,newDay,newTime)
...
{ let appointments2=this.state.appointments.filter(element => element.dentist === this.state.dentist && element.day==this.state.day)
  ...
  let dentistsMapped=this.props.dentists.map(element => <option value={element.firstName}>{element.firstName}</option>)
  ...
  return (  <div>
               ...
               <form> 
                 ...
                 {this.state.wijzigen ? <div>
                                        ...
                                        <div>
                                           <label>dag <input id="day" type="number" name="day" min="1" max="28"/> </label>
