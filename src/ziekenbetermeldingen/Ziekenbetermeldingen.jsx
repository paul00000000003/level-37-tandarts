...
const verwerkZiekmelding = (
  ...
  medewerkersRel.map((element, index) => {
    if (
      element.firstName.toUpperCase() === firstName.toUpperCase() &&
...
const verwijderAppointmentsPatienten = (
  appointments,
  firstName,
  surName,
  isZiek
  ...
  if (isZiek === "J") {
    ...
    appointments.forEach((element, index) => {
      if (element.patient.toUpperCase() === patientNaam) indices.push(index);
    });
    let verschuifstap = 0;
    indices.forEach((index) => {
      appointments.splice(index - verschuifstap, 1);
...
class Ziekenbetermeldingen extends React.Component {
  ...
  handleChangeZiekmelding(e) {
    if (e.target.value === "ziek") this.setState({ isZiek: "J" });
  ...
  ziekmelding(e) {
    ...
    switch (soortPersoon) {
      case "patient":
        persoonZiekmelding = verwerkZiekmelding(
          this.props.patienten,
          firstName,
          surName,
          persoonGevonden,
          persoonIndex
        ...
        if (persoonGevonden !== "J")
          alert(
            "Deze patient staat met deze voor- en achternaam niet in het systeem"
          );
        else if (isZiek === "J")
          verwijderAppointmentsPatienten(
      ...
      case "dentist":
        ...
        break;
      default:
        alert("deze waarde verwacht ik niet");
    }
    setTimeout(() => {
        ...
        <form onSubmit={this.ziekmelding}>
          ...
          <div className="regelcontainer">
            ...
            <input
              ...
              name="isZiek"
              value="ziek"
              onChange={this.handleChangeZiekmelding}
            />
          ...
          <br />
          <button className="buttonVerwerking">
            Verwerk ziek-/betermelding
          </button>
        </form>
